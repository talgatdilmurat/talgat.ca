...
(14) **Hero tagline rewrite + Building nav consistency (index.html, building.html)** — final audit pass surfaced two issues. (a) Hero tagline on index.html still carried the student-coded framing pattern Tal explicitly removed from CityFlow earlier in the session: "Software Engineering graduate building toward municipal systems work" — "graduate" + "building toward" both lean aspirational/emerging rather than present-tense. Rewrote to "Business systems analyst — municipal platforms, spatial data, full-stack" — declarative present-tense, names the target role (BSA-track), the target verticals (municipal platforms, spatial data), and the capability range (full-stack). Matches Tal's LinkedIn positioning. The mono `.hero-title` line above ("GIS, Systems, and the work between them") stays — broad concept; the italic tagline now narrows to specifics, good rhythm. (b) Building page nav button said "Full Resume →" while every other page (index, personal, cityflow, bikeability) said just "Resume →". Unified building.html nav to "Resume →" for consistency.
