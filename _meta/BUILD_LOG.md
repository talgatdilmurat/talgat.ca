...
## [2026-05-29] — Cloudflare Web Analytics beacon on all pages

**What:** Added the Cloudflare Web Analytics beacon to every user-facing page so visits (pageviews, top pages, referrers, region) are tracked now that hosting is on GitHub Pages — GitHub Pages has no built-in analytics, and the old Netlify Analytics stopped collecting when the site moved off Netlify. Inserted the standard beacon snippet (`<script defer src='https://static.cloudflareinsights.com/beacon.min.js' data-cf-beacon='{"token": "…"}'></script>`, wrapped in start/end HTML comments) immediately before the closing `</body>` tag on each page. Cookieless beacon, so no consent banner is needed. No page content, styling, or layout changed — each page gains exactly the 3-line block.
**Files touched:** index.html, resume.html, personal.html, building.html, cityflow.html, bikeability.html, 404.html, _meta/BUILD_LOG.md
**Commit:** `Add Cloudflare Web Analytics beacon to all pages`
**Verification:** `cloudflareinsights` appears exactly once on each of the 7 pages (including the custom 404) and zero times anywhere under `_meta/`; no `PASTE_YOUR_TOKEN_HERE` placeholder remains; `git diff --stat` showed only the 7 HTML files changed at +3 lines each, 0 deletions.
**Notes:** Beacon token (`be9df9cdc78448939f35ec2e97eeb73b`) is the talgat.ca Web Analytics token from the Cloudflare dashboard — not a secret, it ships in the public HTML of every page by design. This entry touches `_meta/` (BUILD_LOG only); `_meta/` is no longer shielded by `netlify.toml`/`_redirects` (both inert on GitHub Pages) but stays hidden because there is no `.nojekyll` file, so Jekyll excludes underscore folders from the build — no public-site impact from the log entry. Two items reported to Tal but deliberately NOT actioned this session (his call): (1) the now-inert `netlify.toml` and `_redirects` can be deleted; (2) `_meta/` privacy now rests solely on Jekyll's default underscore-exclusion — if a `.nojekyll` file is ever added, `_meta/` would become publicly served at talgat.ca/_meta/….
